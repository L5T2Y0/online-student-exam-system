  ...
  try {
    ...
    for (let i = 1; i <= 10; i++) {
      const [student, studentCreated] = await User.findOrCreate({
        ...
        defaults: {
          username: `student${i}`,
          password: '123456',
          name: ['张三', '李四', '王五', '赵六', '钱七', '孙八', '周九', '吴十', '郑一', '王二'][i - 1],
          role: 'student',
          studentId: `202100${i}`,
    ...
    const [paper1, paper1Created] = await Paper.findOrCreate({
      ...
      defaults: {
        ...
        duration: 120,
        questions: [
          { questionId: questions[0].id, score: 10, order: 1 },
          { questionId: questions[1].id, score: 10, order: 2 },
          { questionId: questions[4].id, score: 15, order: 3 },
          { questionId: questions[6].id, score: 5, order: 4 },
          { questionId: questions[7].id, score: 5, order: 5 },
          { questionId: questions[9].id, score: 10, order: 6 },
          { questionId: questions[11].id, score: 20, order: 7 }
        ],
        createdBy: teacher1.id,
        status: 'published',
    ...
    const [paper2, paper2Created] = await Paper.findOrCreate({
      ...
      defaults: {
        ...
        duration: 90,
        questions: [
          { questionId: questions[2].id, score: 15, order: 1 },
          { questionId: questions[3].id, score: 15, order: 2 },
          { questionId: questions[7].id, score: 10, order: 3 },
          { questionId: questions[11].id, score: 30, order: 4 }
        ],
        createdBy: teacher1.id,
        status: 'published',
        publishedAt: new Date()
    ...
    console.log('学生: student1 / 123456 (student1-student10)');
